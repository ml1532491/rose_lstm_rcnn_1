...
class AnchorLoader(mx.io.DataIter):
    def __init__(self, feat_sym, roidb, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
                 ...
                 aspect_grouping=False, seqlen=2, init_states=0):
        """
        This Iter will provide roi data to Fast R-CNN network
        ...
        if config.TRAIN.END2END:
            ...
            self.data_name = ['data']
        self.label_name = ['label', 'bbox_target', 'bbox_weight']
        self.data_name_=self.data_name
        ...
        self.data_name = self.data_name + [init_name[0] for init_name in self.init_states]
        # get first batch to fill in provide_data and provide_label
        self.reset()
        self.get_batch()
    ...
    def get_sub_seqs(self):
        roidbs=[]
        for entry_ser in self.roidb:
            entry_len=len(entry_ser)
            for i in range(0,entry_len,self.seqlen):
                if (i+self.seqlen>entry_len):
                    break
                roidb=entry_ser[i:(i+self.seqlen)]
                roidbs.append(roidb)
        return roidbs
    ...
    def get_batch(self):
        ...
        for islice in slices:
            ...
            iroidb=iroidb[0]
            for entry in iroidb:
                data, label = get_rpn_batch([entry])
                data_list.append(data)
                label_list.append(label)
        ...
        def gen_legal_data(data):
            if data.size==0:
                data_shape=data.shape
                data_shape=(list)(data_shape)
                data_shape[0]=self.batch_size*self.seqlen
                data_shape=tuple(data_shape)
                return mx.nd.zeros(data_shape)
            return mx.nd.array(data)
        self.data = [gen_legal_data(all_data[key]) for key in self.data_name_]+self.init_state_arrays
        self.label = [gen_legal_data(all_label[key]) for key in self.label_name]