...
class PascalVOC(IMDB):
    def __init__(self, seqs_set, root_path, devkit_path):
        """
        fill basic information to initialize imdb
        :param seqs_set: 2007_trainval, 2007_test, etc
        :param root_path: 'selective_search_data' and 'cache'
        :param devkit_path: data and results
        :return: imdb object
        """
        super(PascalVOC, self).__init__('ct_', seqs_set, root_path, devkit_path)  # set self.name
        self.root_path = root_path
        self.devkit_path = devkit_path
        self.data_path = os.path.join(devkit_path, seqs_set)

        self.classes = ['__background__',  # always index 0
                        'nodule','tumor','effusion','calcific',
                        'patchyshadow','stripeshadow','bubble','pgo']
        self.num_classes = len(self.classes)
        self.seqs_set_index = self.load_seqs_set_index()
        self.num_seqs = len(self.seqs_set_index)
        print 'num_seqs', self.num_seqs

        self.config = {'comp_id': 'comp4',
                       'use_diff': False,
                       'min_size': 2}
    def load_seqs_set_index(self):
        """
        find out which indexes correspond to given image set (train or val)
        :return:
        """
        seqs_set_index_file = os.path.join(self.data_path, 'SeqsSets', self.seqs_set + '.txt')
        assert os.path.exists(seqs_set_index_file), 'Path does not exist: {}'.format(seqs_set_index_file)
        with open(seqs_set_index_file) as f:
            seqs_set_index = [x.strip() for x in f.readlines()]
        return seqs_set_index
    ...
    def gt_roidb(self):
        ...
        if os.path.exists(cache_file):
            ...
            return roidb
        gt_roidbs=[]
        annos_path=os.path.join(self.data_path,self.seqs_set+"_anno")
        for index in self.seqs_set_index:
            imgs_path=os.path.join(self.data_path,self.seqs_set+"_sorted",index)
            assert os.path.exists(imgs_path), 'Path does not exist: {}'.format(imgs_path)
            imgs_file=os.listdir(imgs_path)
            imgs_file.sort()
            gt_roidb = [self.load_pascal_annotation(img_file.split('.')[0],imgs_path,annos_path) for img_file in imgs_file]
            gt_roidbs.append(gt_roidb)
        with open(cache_file, 'wb') as fid:
            cPickle.dump(gt_roidbs, fid, cPickle.HIGHEST_PROTOCOL)
        print 'wrote gt roidb to {}'.format(cache_file)

        return gt_roidbs

    def load_pascal_annotation(self, index,imgs_path,annos_path):
        """
        ...
        roi_rec = dict()
        roi_rec['image'] = os.path.join(imgs_path,index+".jpg")
        assert os.path.exists(roi_rec['image']), 'Path does not exist: {}'.format(roi_rec['image'])
        size = cv2.imread(roi_rec['image']).shape
        roi_rec['height'] = size[0]
        roi_rec['width'] = size[1]

        filename = os.path.join(annos_path, index + '.xml')
        if os.path.exists(filename):
            ...
            roi_rec.update({'boxes': None,
                            'gt_classes': np.empty((0,)),
                            'gt_overlaps': np.empty((0,self.num_classes)),
                            'max_classes': np.empty((0,)),
                            'max_overlaps':np.empty((0,)),
                            'flipped':False})
        return roi_rec
    ...
    def get_result_file_template(self):
        ...
        filename = comp_id + '_det_' + self.seqs_set + '_{:s}.txt'
    ...
    def write_pascal_results(self, all_boxes):
        ...
        for cls_ind, cls in enumerate(self.classes):
            ...
            with open(filename, 'wt') as f:
                for im_ind, index in enumerate(self.seqs_set_index):
    ...
    def do_python_eval(self):
        ...
        imageset_file = os.path.join(self.data_path, 'ImageSets', 'Main', self.seqs_set + '.txt')
