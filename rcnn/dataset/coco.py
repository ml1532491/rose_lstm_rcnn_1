...
class coco(IMDB):
    def __init__(self, seqs_set, root_path, data_path):
        """
        fill basic information to initialize imdb
        :param seqs_set: train2014, val2014, test2015
        :param root_path: 'data', will write 'rpn_data', 'cache'
        :param data_path: 'data/coco'
        """
        super(coco, self).__init__('COCO', seqs_set, root_path, data_path)
        ...
        self.data_name = view_map[seqs_set] if seqs_set in view_map else seqs_set

    def _get_ann_file(self):
        """ self.data_path / annotations / instances_train2014.json """
        prefix = 'instances' if 'test' not in self.seqs_set else 'image_info'
        return os.path.join(self.data_path, 'annotations',
                            prefix + '_' + self.seqs_set + '.json')
    ...
    def evaluate_detections(self, detections):
        ...
        res_file = os.path.join(res_folder, 'detections_%s_results.json' % self.seqs_set)
        self._write_coco_results(detections, res_file)
        if 'test' not in self.seqs_set:
    ...
    def _do_python_eval(self, res_file, res_folder):
        ...
        eval_file = os.path.join(res_folder, 'detections_%s_results.pkl' % self.seqs_set)
