...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch,
    ...
    roidb = load_gt_roidb(args.dataset, args.image_set, args.root_path, args.dataset_path)
    ...
    # load training data
    init_c = [('l%d_init_c' % l, (input_batch_size*config.seqlen, config.num_hidden,32,32)) for l in range(2)]
    init_h = [('l%d_init_h' % l, (input_batch_size*config.seqlen, config.num_hidden,32,32)) for l in range(2)]
    init_states = init_c + init_h
    train_data = AnchorLoader(feat_sym, roidb, batch_size=input_batch_size, shuffle=not args.no_shuffle,
                              ctx=ctx, work_load_list=args.work_load_list,
                              feat_stride=config.RPN_FEAT_STRIDE, anchor_scales=config.ANCHOR_SCALES,
                              anchor_ratios=config.ANCHOR_RATIOS, aspect_grouping=config.TRAIN.ASPECT_GROUPING,init_states=init_states,seqlen=config.seqlen)

    # infer max shape
    max_data_shape = [('data', (input_batch_size*config.seqlen, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES])))]
    max_data_shape, max_label_shape = train_data.infer_shape(max_data_shape)
    max_data_shape.append(('gt_boxes', (input_batch_size*config.seqlen, 100, 5)))
    ...
    if args.resume:
        ...
        arg_params['l0_i2h_weight'] = mx.random.normal(0, 0.01, shape=arg_shape_dict['l0_i2h_weight'])
        arg_params['l0_i2h_bias'] = mx.nd.zeros(shape=arg_shape_dict['l0_i2h_bias'])
        arg_params['l0_h2h_weight'] = mx.random.normal(0, 0.01, shape=arg_shape_dict['l0_h2h_weight'])
        arg_params['l0_h2h_bias'] = mx.nd.zeros(shape=arg_shape_dict['l0_h2h_bias'])
        arg_params['l1_i2h_weight'] = mx.random.normal(0, 0.01, shape=arg_shape_dict['l1_i2h_weight'])
        arg_params['l1_i2h_bias'] = mx.nd.zeros(shape=arg_shape_dict['l1_i2h_bias'])
        arg_params['l1_h2h_weight'] = mx.random.normal(0, 0.01, shape=arg_shape_dict['l1_h2h_weight'])
        arg_params['l1_h2h_bias'] = mx.nd.zeros(shape=arg_shape_dict['l1_h2h_bias'])
